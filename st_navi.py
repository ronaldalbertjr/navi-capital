...
global_esg = esg_scores[esg_scores.aspect.eq('S&P Global ESG Score')]
df_companies = df_companies[df_companies.company_id.isin(global_esg.company_id.values)]
...
def get_esg_score(company_id):
    esg_score = global_esg[global_esg.company_id.eq(company_id)]
...
def generate_esg_fin_graph(esg_df, fin_df):
    ...
    fig.update_layout(
                title="ESG Score X Indicador Financeiro"
            )
    return fig
...
st.write("""# ESG Score""")
esg_score = get_esg_score(company_id)
